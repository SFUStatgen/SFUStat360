...
* Calling `document()` will create the .Rd file from your comments
and add `mars()` to the mars NAMESPACE file.
...
* Your package's reliance on other packages can be as 
    ...
    * "Imports" -- you need to call functions from the NAMESPACE of another
    package. Add these to the NAMESPACE with `use_package()` and to
    the DESCRIPTION file yourself.
